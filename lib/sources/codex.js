...
const { tallyCommand } = require('../cmdtally');
const { countText, addMeasurement } = require('../text-tokens');
...
// Per-command timestamps are kept for the life of the session, not just a recent
// window: they are what lets an hour of an old day be broken down by when each
// command actually ran. The cap is a floor on pathological sessions only —
// trimming from the front would drop the earliest commands, which are the ones
// carried longest in context and so the most expensive to lose.
const CMD_CAP = 8000;

module.exports = function createCodexSource({ argVal, localTokens = false }) {
  ...
  function emptySummary(filePath) {
    return {
      ...
      cmdTally: new Map(),   // base command -> whole-session totals (survives the ring buffer)
      tokClasses: { i: 0, cc: 0, cr: 0, o: 0 },  // billed classes, whole session (cc unused: no write premium)
      taskActive: false,
    };
  }
  ...
  function applyLine(sum, raw) {
    ...
    if (o.type === 'event_msg' && p.type === 'token_count') {
      ...
      if (info.total_token_usage) {
        ...
        const inputNew = Math.max(0, (lastUsage.input_tokens || 0) - cacheRead);
        const output = lastUsage.output_tokens || 0;
        sum.lastReqNewTokens = inputNew + output;
        sum.lastReqNewInput = inputNew; sum.lastReqOutput = output;
        sum.cumReqTokens = (sum.cumReqTokens || 0) + sum.lastReqTokens;
        if (compacted) sum.compactions = (sum.compactions || 0) + 1;
        // same four buckets the Claude parser emits, minus cache-write — Codex
        // does not bill cache creation separately, so that segment is absent
        sum.tokenSeries.push({ t: ts, total: newTotal, last: sum.lastReqTokens, cum: sum.cumReqTokens, reset: compacted || undefined,
          i: inputNew, cr: cacheRead, o: output });
        // whole-session class totals — the series is capped, cost is not
        sum.tokClasses.i += inputNew; sum.tokClasses.cr += cacheRead; sum.tokClasses.o += output;
    ...
    if (o.type === 'response_item') {
      ...
      if (p.type === 'custom_tool_call' || p.type === 'function_call') {
        ...
        const entry = { ts, name: p.name || p.type, cmd: extractCmd(p.name, input).slice(0, 400),
          ...
          cls: { i: sum.lastReqNewInput || 0, cc: 0, o: sum.lastReqOutput || 0 } };
        sum.lastExec = { name: entry.name, first: entry.cmd, ts };
        tallyCommand(sum, baseCommand(entry), entry);
  ...
  function scanSession(file, st) {
    return new Promise((resolve) => {
      const rec = {
        ...
        // economy signals
        toolCalls: 0, pollTurns: 0, outTokens: 0, textTokenMeasurements: {},
      ...
      let lastCls = { i: 0, cc: 0, o: 0 };   // last request's class split, for per-command cost
      const seq = [];
      const pending = new Map();   // call_id -> { base, cmd, cap, poll }
      const pendingSeq = new Map();  // call_id -> index into seq, to fold its output back in
      ...
      rl.on('line', (line) => {
        ...
        if (o.type === 'session_meta') {
          ...
          // Bind output to this turn's model, never the session's first model.
          currentModel = p.model === 'codex-auto-review' ? null : (p.model || null);
          ...
          // per-request class split, carried to the next tool call the way the
          // live parser does — cc stays 0: OpenAI has no cache-write charge
          const lu = p.info.last_token_usage || {};
          const luRead = lu.cached_input_tokens || 0;
          lastCls = { i: Math.max(0, (lu.input_tokens || 0) - luRead), cc: 0, o: lu.output_tokens || 0 };
          ...
          seq.push({ base, total: lastTotal, ts, out: 0, trunc: 0, cmd: cmd.slice(0, 200), cls: { ...lastCls } });
          if (p.call_id) pendingSeq.set(p.call_id, seq.length - 1);
          rec.toolCalls++;
          ...
          if (p.call_id) pending.set(p.call_id, { base, cmd, cap, model: currentModel, poll: isPoll, sid: sidM ? sidM[1] : null });
          ...
          const seqIdx = p.call_id != null ? pendingSeq.get(p.call_id) : undefined;
          pendingSeq.delete(p.call_id);
          ...
          const measurement = countText(text, { model: call ? call.model : null, local: localTokens });
          addMeasurement(rec.textTokenMeasurements, measurement);
          // Original-output markers describe bytes NOT delivered in this logged result.
          // Keep them as separate evidence; never count hidden text as consumed text.
          const t = measurement.tokens;
          rec.outTokens += t;
          ...
          if (seqIdx !== undefined && seq[seqIdx]) {
            seq[seqIdx].textTokens = measurement;
            if (tm) seq[seqIdx].originalOutputTokens = +tm[1];
          }
          if (truncated) g.truncated++;
          if (seqIdx !== undefined && seq[seqIdx]) { seq[seqIdx].out += t; if (truncated) seq[seqIdx].trunc = 1; }
          ...
          if (call && call.cmd) {
            if (call.poll && call.sid && sessionCmd.has(call.sid)) {
              ...
              pt.count++; pt.out += t;
              // the short-window views group polls by the process, not the poll call
              if (seqIdx !== undefined && seq[seqIdx]) seq[seqIdx].tgt = tgt;
            }
      ...
      rl.on('close', () => {
        ...
        const agg = new Map();
        let prev = null;
        // commands from the last day keep their own timestamp, so short-window
        // views can bucket by when a command actually ran rather than lumping a
        // whole session into its start time
        for (const c of seq) {
          let d = 0;
          if (prev != null && c.total >= prev) d = c.total - prev;
          if (c.total > 0) prev = c.total;
          const g = agg.get(c.base) || { base: c.base, tokens: 0, count: 0, cls: { i: 0, cc: 0, o: 0 } };
          g.tokens += d; g.count++;
          const cc2 = c.cls || {};
          g.cls.i += cc2.i || 0; g.cls.cc += cc2.cc || 0; g.cls.o += cc2.o || 0;
          agg.set(c.base, g);
          if (c.ts) {
            rec.recentCmds.push({ t: c.ts, b: c.base, k: d, o: c.out, tr: c.trunc, c: c.cmd, g: c.tgt || null,
              textTokens: c.textTokens || null, originalOutputTokens: c.originalOutputTokens ?? null });
          }
        }
        if (rec.recentCmds.length > CMD_CAP) rec.recentCmds = rec.recentCmds.slice(-CMD_CAP);
        rec.cmds = [...agg.values()];
  ...
  return {
    id: 'codex',
    // per-command timestamps go back as far as the transcript does
    recentCmdMs: null,
    label: 'Codex CLI',
    root: CODEX_ROOT,
    rootWhy: ROOT_WHY,
