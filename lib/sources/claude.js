...
// kept for the whole session, not a recent window — see codex.js
const CMD_CAP = 8000;

module.exports = function createClaudeSource({ argVal, localTokens = false }) {
  ...
  function emptySummary(filePath) {
    return {
      ...
      tokClasses: { i: 0, cc: 0, cr: 0, o: 0 },  // billed classes, whole session
      taskActive: false,
  ...
  function applyLine(sum, raw) {
    ...
    if (o.type === 'assistant') {
      ...
      if (msg.usage) {
        ...
        // i/cc/cr/o are the four disjoint buckets that make up `last`, kept short
        // because the series ships per-point: new input, cache write, cache read, output
        sum.tokenSeries.push({ t: ts, total: sum.lastTotalTokens, last: sum.lastReqTokens, cum: sum.cumReqTokens,
          i: inputNew, cc: cacheCreate, cr: cacheRead, o: output });
        if (sum.tokenSeries.length > 4000) sum.tokenSeries.shift();
        // whole-session class totals: the series is capped, and cost has to be
        // computed over every request, not the last 4000
        const tc = sum.tokClasses;
        tc.i += inputNew; tc.cc += cacheCreate; tc.cr += cacheRead; tc.o += output;
      }
      ...
      // the same even split, kept per class so a command's cost can be priced
      // at each class's own rate rather than a blended one
      const share = (v) => (toolUses.length ? v / toolUses.length : 0);
      const cls = msg.usage
        ? { i: share(msg.usage.input_tokens || 0),
            cc: share(msg.usage.cache_creation_input_tokens || 0),
            o: share(msg.usage.output_tokens || 0) }
        : { i: 0, cc: 0, o: 0 };
      for (const c of toolUses) {
        ...
        const entry = { ts, name: c.name || 'tool', cmd,
          ...
          newTokens: Math.round(perToolNew), cls, turn: sum.curTurn || 0 };
    ...
    return new Promise((resolve) => {
      ...
      rl.on('line', (line) => {
        ...
        if (o.type === 'user') {
          ...
          for (const it of (Array.isArray(content) ? content : [])) {
            ...
            if (call && call.ri !== undefined && rec.recentCmds[call.ri]) {
              ...
              rec.recentCmds[call.ri].textTokens = measurement;
        ...
        if (o.type === 'assistant') {
          ...
          // the same even split kept per billing class, so a command's cost can
          // be priced at each class's own rate downstream
          const u2 = msg.usage || {};
          const shr = (v) => (toolUses.length ? (v || 0) / toolUses.length : 0);
          const perToolCls = { i: shr(u2.input_tokens), cc: shr(u2.cache_creation_input_tokens), o: shr(u2.output_tokens) };
          for (const c of toolUses) {
            rec.toolCalls++;
            const cmd = extractCmd(c.name, c.input);
            const base = baseCommand({ name: c.name, cmd });
            const g = cmdAgg.get(base) || { base, tokens: 0, count: 0, cls: { i: 0, cc: 0, o: 0 } };
            g.tokens += perToolNew; g.count++;
            g.cls.i += perToolCls.i; g.cls.cc += perToolCls.cc; g.cls.o += perToolCls.o;
            cmdAgg.set(base, g);
            let ri;
            if (o.timestamp) {
              ri = rec.recentCmds.push({ t: o.timestamp, b: base, k: perToolNew, o: 0, tr: 0, c: String(cmd || '').slice(0, 200) }) - 1;
            }
            if (c.id) pending.set(c.id, { base, cmd, ri, model: currentModel });
          }
        }
      });
      rl.on('close', () => {
